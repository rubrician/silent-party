...
public class JcAudioPlayer {
    public static final String TAG = "JcAudioPlayer";
    ...
    public void playAudioHack(JcAudio JcAudio) throws AudioListNullPointerException {
        if (playlist == null || playlist.size() == 0)
            throw new AudioListNullPointerException();
        ...
        for (int i = 0; i < playlist.size(); i++) {
            if (playlist.get(i).getTrackId().equals(currentJcAudio.getTrackId()))
                this.currentPositionList = i - 1;
        }
    ...
    public void nextAudio() throws AudioListNullPointerException {
        if (playlist == null || playlist.size() == 0)
            ...
            Log.d(TAG, "Playing next " + this.currentJcAudio.getTitle());
            updatePositionAudioList();
            playing = true;
            paused = false;
        }
    ...
    public void previousAudio() throws AudioListNullPointerException {
        if (playlist == null || playlist.size() == 0)
            ...
            Log.d(TAG, "Playing previous " + this.currentJcAudio.getTitle());
            updatePositionAudioList();
            playing = true;
            paused = false;
        }
    ...
    public void continueAudio() throws AudioListNullPointerException {
        if (playlist == null || playlist.size() == 0)
            ...
            playing = true;
            paused = false;
            Log.d(TAG, "Continuing  " + this.currentJcAudio.getTitle());
        }
    }
    ...
    public int getCurrentTime() {
        return jcPlayerService.getCurrentTime();
    }
}
